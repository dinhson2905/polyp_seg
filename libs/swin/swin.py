...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=352, patch_size=4, in_chans=3, num_classes=1000,
                    ...
                    window_size=11, mlp_ratio=4., qkv_bias=True, qk_scale=None,
    ...
    def extract_features(self, x):
        ...
        for i, layer in enumerate(self.layers):
            ...
            if i in [1, 2, 3]:
                ...
                H, W = int(np.sqrt(extract_feature.shape[2])), int(np.sqrt(extract_feature.shape[2]))
                extract_feature = extract_feature.contiguous().view(B, C, H, W)
                out_branch.append(extract_feature)
