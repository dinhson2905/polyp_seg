...
class HarDNet(nn.Module):
    def __init__(self, depth_wise=False, arch=85, pretrained=True, weight_path=''):
        ...
        for i in range(blks):
            ...
            # output Conv1x1 follow HarDBlock[i] (124, 256, 320, 640, 1)
...
def hardnet(arch=68, pretrained=True, *args, **kwargs):
    if arch == 68:
        print("HarDNet68 is loaded")
        model = HarDNet(arch=68)
        if pretrained:
            weights = torch.load('./libs/weights/hardnet/hardnet68.pth')
            model.load_state_dict(weights)
            print("HarDNet68 loaded ready ")
