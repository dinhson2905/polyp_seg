...
from libs.hardmseg import HarDCPD
...
def validation(model, data_path, data2_path):
    ...
    print(b2 / validation2_loader.size)    
    ...
    for epoch in range(94, epochs):
        ...
        for i, pack in enumerate(train_loader, start=1):
            ...
            if i%20 == 0 or i == total_step:
                print(f'{datetime.now()} Epoch [{epoch}/{epochs}], Step [{i}/{total_step}], Loss: [{loss1_record.show()}, {loss2_record.show()}]')
                train_logger.info(f'{datetime.now()} Epoch [{epoch}/{epochs}], Step [{i}/{total_step}], Loss: [{loss1_record.show()}, {loss2_record.show()}]')
        ...
        if (epoch+1) % 1 == 0:
            ...
            if meandice > best_dice_score:
                ...
                torch.save(model.state_dict(), save_path + 'hardcpd_0603.pth')
                print('[Saving Snapshots:]', save_path + 'hardcpd0603.pth', meandice)
...
if __name__ == '__main__':
    ...
    weights = torch.load('./checkpoints/hardcpd.pth')
