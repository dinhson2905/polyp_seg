...
class HausdorffDTLoss(nn.Module):
    ...
    def forward(self, pred: torch.Tensor, target: torch.Tensor, debug:False) -> torch.Tensor:
        """Uses binary channel: 1-fg, 0-bg
        pred (torch.Tensor): (b, 1, x,y,z) or (b, 1, x,y)
        target (torch.Tensor): (b, 1, x,y,z) or (b, 1, x,y)
        """
        assert pred.dim() == 4 or pred.dim() == 5, "Only 2D or 3D supported"
        ...
        distance = pred_dt ** self.alpha + target ** self.alpha
    ...
    def forward(self, pred: torch.Tensor, target: torch.Tensor, debug=False) -> torch.Tensor:
        ...
        if debug:
            ...
            eroted = torch.from_numpy(
                self.perform_erosion(pred.cpu().numpy(), target.cpu().numpy(), debug)).float()
            loss = eroted.mean()
            return loss